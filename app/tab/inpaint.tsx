...
export default function InpaintScreen() {
  ...
  const showInput = () =>
    Animated.spring(inputAnim, { toValue: 1, useNativeDriver: true, tension: 60, friction: 10 }).start();

  /* ── pickImage ── */
  const pickImage = async () => {
    const { status } = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (status !== "granted") { Alert.alert("Permission requise", "Active l'accès à la galerie."); return; }
    const res = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ["images"], quality: 0.7, base64: true });
    if (!res.canceled && res.assets[0]) {
      setImageUri(res.assets[0].uri);
      setImageBase64(res.assets[0].base64 ?? null);
      setDisplayUri(res.assets[0].uri);
      setAttachUri(null); setAttachB64(null);
      setStrokes([]); setTool(null); toolRef.current = null;
      setPrompt(""); inputAnim.setValue(0); showInput();
    }
  };
  ...
  [tool, commitPath]);  // reconstruit seulement quand l'outil change
  ...
  const runInpaint = async () => {
    ...
    try {
      ...
      const maskNote = strokes.length > 0
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              ...(attachB64
                ? [{ type: "image_url", image_url: { url: `data:image/jpeg;base64,${attachB64}`, detail: "low" } }]
                : []),
              {
                type: "text",
      ...
      if (!vResp.ok) throw new Error(vData.error?.message ?? `Vision ${vResp.status}: ${JSON.stringify(vData)}`);
      const genPrompt: string = vData.choices?.[0]?.message?.content?.trim() ?? prompt;
      ...
      if (!iResp.ok) throw new Error(iData.error?.message ?? `Generation error ${iResp.status}`);

      const resultUrl: string | undefined =
        iData.data?.[0]?.url ??
        (iData.data?.[0]?.b64_json ? `data:image/png;base64,${iData.data[0].b64_json}` : undefined);
  ...
  const reset = () => {
    setImageUri(null); setImageBase64(null); setDisplayUri(null);
    setAttachUri(null); setAttachB64(null);
    setStrokes([]); setTool(null); toolRef.current = null;
    setPrompt(""); inputAnim.setValue(0);
  };
  ...
  const translateY = inputAnim.interpolate({ inputRange: [0, 1], outputRange: [120, 0] });
      ...
      {!imageUri ? (
        <View style={styles.centerWrap}>
          ...
          <TouchableOpacity onPress={pickImage} activeOpacity={0.82} style={styles.addButton}>
            <NeutralBlurView pointerEvents="none" style={styles.addButtonBlur} intensity={50} {...bgBlur} />
        ...
        <>
          <View style={styles.imageWrap}>
            <Animated.Image source={{ uri: displayUri ?? imageUri }} style={[styles.image, { opacity: imageOpacity }]} resizeMode="cover" />

            {/* Référence attachée */}
            {attachUri && (
              <View style={[styles.attachWrap, { bottom: insets.bottom + 110 }]}>
                <Image source={{ uri: attachUri }} style={styles.attachImg} resizeMode="cover" />
                <TouchableOpacity style={styles.attachClose} onPress={() => { setAttachUri(null); setAttachB64(null); }}>
            ...
            <GestureDetector gesture={drawGesture}>
              <View style={StyleSheet.absoluteFillObject} collapsable={false}>
                <Svg style={StyleSheet.absoluteFillObject}>
                  <Defs>
                    <Filter id="magicGlow" x="-70%" y="-70%" width="240%" height="240%">
                      <FeGaussianBlur in="SourceAlpha" stdDeviation="12" result="bigBlur" />
                      <FeFlood floodColor="#FFB300" floodOpacity="0.92" result="amber" />
                      <FeComposite in="amber" in2="bigBlur" operator="in" result="amberGlow" />
                      <FeGaussianBlur in="SourceAlpha" stdDeviation="4" result="innerBlur" />
                      <FeFlood floodColor="#FFFFFF" floodOpacity="0.5" result="white" />
            ...
            {displayUri && displayUri !== imageUri && (
              <TouchableOpacity onPress={downloadImage} style={[styles.downloadBtn, { top: insets.top + 14 }]}>
          ...
          {/* Barre outils + input */}
          <KeyboardAvoidingView style={styles.inputLayer} behavior={Platform.OS === "ios" ? "padding" : "height"}>
            <Animated.View style={[styles.inputWrap, { marginBottom: insets.bottom + 16, opacity, transform: [{ translateY }] }]}>

              <View style={styles.toolRow}>
                <TouchableOpacity onPress={pickAttach} style={[styles.toolBtn, attachUri && styles.toolBtnAttachActive]} activeOpacity={0.8}>
                  <Ionicons name="attach" size={20} color={attachUri ? "#FFE700" : C.cream} />
                  <Text style={[styles.toolLabel, attachUri && { color: "#FFE700" }]}>Attacher</Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={() => selectTool("magic")} style={[styles.toolBtn, tool === "magic" && styles.toolBtnMagicActive]} activeOpacity={0.8}>
                  <Ionicons name="sparkles" size={20} color={tool === "magic" ? "#FF3CAC" : C.cream} />
                  <Text style={[styles.toolLabel, tool === "magic" && { color: "#FF3CAC" }]}>Magic</Text>
                </TouchableOpacity>

                <TouchableOpacity onPress={() => selectTool("brush")} style={[styles.toolBtn, tool === "brush" && styles.toolBtnBrushActive]} activeOpacity={0.8}>
                  <Ionicons name="brush" size={20} color={tool === "brush" ? "#00BFFF" : C.cream} />
                  <Text style={[styles.toolLabel, tool === "brush" && { color: "#00BFFF" }]}>Simple</Text>
                </TouchableOpacity>

                {strokes.length > 0 && (
                  <TouchableOpacity onPress={() => setStrokes([])} style={styles.toolBtn} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  container: { ...StyleSheet.absoluteFillObject, backgroundColor: "#07112B" },

  centerWrap: { flex: 1, alignItems: "center", justifyContent: "center" },
  emptyHint:  { fontSize: 15, color: "rgba(196,199,208,0.9)", marginBottom: 36, textAlign: "center" },
  addButton: {
    width: 140, height: 140, borderRadius: 36, overflow: "hidden",
    alignItems: "center", justifyContent: "center",
    borderWidth: 1.5, borderColor: C.border, backgroundColor: "rgba(255,255,255,0.09)",
  },
  ...
  changeBtn:   { position: "absolute", right: 52, borderRadius: 20, overflow: "hidden" },
  downloadBtn: { position: "absolute", left: 12,  borderRadius: 20, overflow: "hidden" },
  downloadBtnBlur: {
    flexDirection: "row", alignItems: "center",
    paddingHorizontal: 12, paddingVertical: 7,
    borderWidth: 1, borderColor: "rgba(0,191,255,0.4)",
    backgroundColor: "rgba(0,191,255,0.12)", gap: 5,
  },
  downloadBtnText: { color: "#00BFFF", fontSize: 12, fontWeight: "600" },
  changeBtnBlur: {
    flexDirection: "row", alignItems: "center",
    paddingHorizontal: 12, paddingVertical: 7,
    borderWidth: 1, borderColor: C.border, backgroundColor: "rgba(255,255,255,0.09)", gap: 5,
  },
  changeBtnText: { color: C.cream, fontSize: 12, fontWeight: "500" },
  resetBtn: { position: "absolute", right: 12, borderRadius: 20, overflow: "hidden" },
  resetBtnBlur: { padding: 8, borderWidth: 1, borderColor: C.border, backgroundColor: "rgba(255,255,255,0.09)" },
  ...
  toolBtn: {
    ...
    backgroundColor: "rgba(255,255,255,0.07)", gap: 3,
  },
  toolBtnAttachActive: { borderColor: "rgba(255,231,0,0.55)", backgroundColor: "rgba(255,231,0,0.1)" },
  toolBtnMagicActive:  { borderColor: "rgba(255,60,172,0.55)", backgroundColor: "rgba(255,60,172,0.1)" },
  toolBtnBrushActive:  { borderColor: "rgba(0,191,255,0.55)",  backgroundColor: "rgba(0,191,255,0.1)" },
  ...
  inputBlur: {
    ...
    paddingLeft: 18, paddingRight: 6, paddingVertical: 8,
  },
  input:          { flex: 1, color: C.cream, fontSize: 15, paddingVertical: 8, minHeight: 42 },
  genBtn:         { backgroundColor: C.coffee, borderRadius: 20, paddingHorizontal: 18, paddingVertical: 10, marginLeft: 8 },
